# ADR 0010: Kontrakt jednolitej przestrzeni współrzędnych (Coordinate Space Contract)
...
2. Czy nowy kod dodaje funkcję konwertującą współrzędne poza `screenToImageSpace`/`currentScale`? — jeśli tak, prawdopodobnie ktoś na nowo wprowadza ręczne liczenie letterboxingu, które już raz zostało wyeliminowane jako źródło błędów. Zatrzymać się i zapytać człowieka.
...
Zaakceptowane. Obowiązuje od 2026-06-29. Powiązane z testami jednostkowymi w `__tests__/geometry.test.ts` (patrz: kolejny ADR lub commit), które automatyzują weryfikację punktów 1-3 z sekcji powyżej dla znanych przypadków testowych (marker referencyjny + dwa punkty pomiarowe → oczekiwana wartość w cm).
